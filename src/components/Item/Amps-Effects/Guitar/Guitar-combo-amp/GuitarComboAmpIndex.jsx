...
import { getGuitarComboAmpCatalogByPagination, getGuitarComboAmpTableCount, getGuitarComboAmpCatalogByBrand, getGuitarComboAmpBrandsByIds } from '../../../../../redux/amps-effects/guitar/guitar-combo-amp/guitar-combo-amp-redux';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faCaretRight, faCaretLeft } from '@fortawesome/free-solid-svg-icons';
import { isProductCheckedByBrand, isCount, pageSize, isNextPage, displayPages } from '../../../item-functions';

const GuitarComboAmpIndex = () => {
    ...
    const totalSizeCatalog = useSelector(store => store.guitarComboAmpCatalog.totalSizeCatalog);
    const sizeCatalogBrand = useSelector(store => store.guitarComboAmpCatalog.sizeCatalogByBrand);
    const brands = useSelector(store => store.guitarComboAmpCatalog.brands);
    const [totalPages, setTotalPages] = useState(0);
    const [limit] = useState(10);
    const [offset, setOffset] = useState(0);
    const [checked, setChecked] = useState([]);
    const [brandIds] = useState([3, 6, 7, 9]);

    useEffect(() => {
        dispatch(getGuitarComboAmpBrandsByIds(brandIds));
        dispatch(getGuitarComboAmpTableCount());
        if (isProductCheckedByBrand(checked)) {
          isCount(totalSizeCatalog) && dispatch(getGuitarComboAmpCatalogByPagination(offset, limit));
      ...
      <div className="d-flex">
        ...
        <div>
          <div className="products">
            ...
            <ProductCard catalog={guitarComboAmp}/>
...
export default GuitarComboAmpIndex;
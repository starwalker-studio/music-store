...
import { getBassAmpCabinetCatalogByPagination, getBassAmpCabinetTableCount, getBassAmpCabinetCatalogByBrand, getBassAmpCabinetBrandsByIds } from '../../../../../redux/amps-effects/bass/bass-amp-cabinets/bass-amp-cabinet-redux';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faCaretRight, faCaretLeft } from '@fortawesome/free-solid-svg-icons';
import { isProductCheckedByBrand, isCount, pageSize, isNextPage, displayPages } from '../../../item-functions';

const BassAmpCabinetIndex = () => {
  ...
  const totalSizeCatalog = useSelector(store => store.bassAmpCabinetCatalog.totalSizeCatalog);
  const sizeCatalogBrand = useSelector(store => store.bassAmpCabinetCatalog.sizeCatalogByBrand);
  const brands = useSelector(store => store.bassAmpCabinetCatalog.brands);
  const [totalPages, setTotalPages] = useState(0);
  const [limit] = useState(10);
  const [offset, setOffset] = useState(0);
  const [checked, setChecked] = useState([]);
  const [brandIds] = useState([1, 3, 5, 7]);

  useEffect(() => {
    dispatch(getBassAmpCabinetBrandsByIds(brandIds));
    dispatch(getBassAmpCabinetTableCount());
    if (isProductCheckedByBrand(checked)) {
      isCount(totalSizeCatalog) && dispatch(getBassAmpCabinetCatalogByPagination(offset, limit));
      ...
      <div className="d-flex">
        ...
        <div>
          <div className="products">
            ...
            <ProductCard catalog={bassAmpCabinets}/>
...
export default BassAmpCabinetIndex;
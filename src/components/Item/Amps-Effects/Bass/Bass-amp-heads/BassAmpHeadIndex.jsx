...
import { getBassAmpHeadCatalogByPagination, getBassAmpHeadTableCount, getBassAmpHeadCatalogByBrand, getBassAmpHeadBrandsByIds } from '../../../../../redux/amps-effects/bass/bass-amp-heads/bass-amp-head-redux';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faCaretRight, faCaretLeft } from '@fortawesome/free-solid-svg-icons';
import { isProductCheckedByBrand, isCount, pageSize, isNextPage, displayPages } from '../../../item-functions';

const BassAmpHeadIndex = () => {
    ...
    const totalSizeCatalog = useSelector(store => store.bassAmpHeadCatalog.totalSizeCatalog);
    const sizeCatalogBrand = useSelector(store => store.bassAmpHeadCatalog.sizeCatalogByBrand);
    const brands = useSelector(store => store.bassAmpHeadCatalog.brands);
    const [totalPages, setTotalPages] = useState(0);
    const [limit] = useState(10);
    const [offset, setOffset] = useState(0);
    const [checked, setChecked] = useState([]);
    const [brandIds] = useState([1, 2, 4, 5, 7]);

    useEffect(() => {
        dispatch(getBassAmpHeadBrandsByIds(brandIds));
        dispatch(getBassAmpHeadTableCount());
        if (isProductCheckedByBrand(checked)) {
          isCount(totalSizeCatalog) && dispatch(getBassAmpHeadCatalogByPagination(offset, limit));
      ...
      <div className="d-flex">
        ...
        <div>
          <div className="products">
            ...
            <ProductCard catalog={bassAmpHeads}/>
...
export default BassAmpHeadIndex;